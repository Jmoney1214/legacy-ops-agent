...
Add or merge these rules into the repository-root `AGENTS.md`.

## Phase control

- Work only the phase marked current in `docs/build/PHASE_STATUS.yaml`.
- Read the corresponding file under `docs/build/phases/`.
...
Never push a knowingly broken checkpoint.
...
- Keep fixes on the same PR.
- Request automated Codex review when configured.
- STANDARD requires one independent human approval.
- HIGH and CRITICAL require two independent human reviews.
- Resolve every review conversation.
- Rerun the full required gate after review fixes.
...
- HIGH production releases and all CRITICAL work require explicit owner release approval.
- After merge, verify main CI and staging before declaring complete.

## Reporting

Every completion report must include phase, branch, commits, PR, changed files, migrations, tests/evals, pre-push review, CI, code review, staging, security, blockers, rollback, and next action.
